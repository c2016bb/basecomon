...
public class DefaltDividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public DefaltDividerItemDecoration(Context context, int orientation)
    {
...
//              outRect.set(delegete.getItemLeftWidth(),delegete.getItemTopWidth(),delegete.getItemRightWidth(),delegete.getItemBottomWidth());
            if (mOrientation==VERTICAL) {
                ...
                if (isLastColum(parent, itemPosition, spanCount, childCount))// 如果是最后一行，则不需要绘制底部
                {
                    outRect.set(0, 0, mDivider.getIntrinsicWidth(), 0);
                } else if (isLastRaw(parent, itemPosition, spanCount, childCount))// 如果是最后一列，则不需要绘制右边
                {
                    outRect.set(0, 0, 0, mDivider.getIntrinsicHeight());
                } else {
                    outRect.set(0, 0, mDivider.getIntrinsicWidth(),
